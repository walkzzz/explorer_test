...
class DriverFactory:
    ...
    def get_window(cls, title_re=".*", class_name=None):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"All windows on desktop: {[w.window_text() for w in all_windows]}"
                )
    ...
    def take_screenshot(cls, name="screenshot"):
        ...
        if cls._window:
            ...
            # 使用Windows临时目录，添加默认值处理None情况
            temp_dir = os.getenv("TEMP", os.getcwd())
            screenshot_path = os.path.join(temp_dir, f"{name}.png")
